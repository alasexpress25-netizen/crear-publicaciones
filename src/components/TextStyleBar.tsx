...
interface TextStyleBarProps {
  ...
  onPreviewAnimation?: (key: string, duration?: number) => void;
}
...
export const TextStyleBar: React.FC<TextStyleBarProps> = ({
  ...
  const [openSubmenu, setOpenSubmenu] = useState<'move' | 'layers' | 'outline' | 'shadow' | 'background' | 'animation' | null>(null);
  const [animTab, setAnimTab] = useState<'in' | 'out'>('in');
  const [isPlayingAnimPreview, setIsPlayingAnimPreview] = useState(false);
  ...
  const customTextLayer = activeKey ? slide.customTexts?.find((c) => c.id === activeKey) : undefined;
  const currentInEffect: ElementAnimationIn = (customTextLayer?.animationIn || currentItemStyle?.animationIn || 'none');
  const currentInDelay = (customTextLayer?.inDelay !== undefined ? customTextLayer.inDelay : currentItemStyle?.inDelay !== undefined ? currentItemStyle.inDelay : 0);
  const currentInDuration = (customTextLayer?.animationInDuration || currentItemStyle?.animationInDuration || 0.5);
  const currentOutEffect: ElementAnimationOut = (customTextLayer?.animationOut || currentItemStyle?.animationOut || 'none');
  const currentOutTime = (customTextLayer?.outTime !== undefined ? customTextLayer.outTime : currentItemStyle?.outTime !== undefined ? currentItemStyle.outTime : (slide.duration || 3.5));
  const currentOutDuration = (customTextLayer?.animationOutDuration || currentItemStyle?.animationOutDuration || 0.5);
  const isAnimActive = currentInEffect !== 'none' || currentOutEffect !== 'none';
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-2.5 sm:p-3 shadow-xl space-y-2.5">
      ...
      {activeKey ? (
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-1.5 bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 shrink-0">
                    {/* Badge selector: Texto / Relleno / Efecto */}
                    <div className="flex items-center bg-slate-900 border border-slate-800 rounded-md p-0.5 shrink-0">
                      <button
                        type="button"
                        onClick={() => {
                          setColorTargetMode('text');
                          if (openSubmenu === 'animation') setOpenSubmenu(null);
                        }}
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold transition ${
                          colorTargetMode === 'text' && openSubmenu !== 'animation'
                            ? 'bg-rose-600 text-white shadow-sm'
                            : 'text-slate-400 hover:text-slate-200'
                        }`}
                        title="Modo Texto: Cambiar color del texto seleccionado"
                      >
                        Texto
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          setColorTargetMode('fill');
                          if (openSubmenu === 'animation') setOpenSubmenu(null);
                        }}
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 ${
                          colorTargetMode === 'fill' && openSubmenu !== 'animation'
                            ? 'bg-rose-600 text-white shadow-sm'
                            : 'text-slate-400 hover:text-slate-200'
                        }`}
                        title="Modo Relleno: Cambiar color de fondo / relleno del recuadro del elemento"
                      >
                        <span
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setOpenSubmenu((prev) => (prev === 'animation' ? null : 'animation'));
                        }}
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 ${
                          openSubmenu === 'animation'
                            ? 'bg-amber-600 text-white shadow-sm'
                            : isAnimActive
                            ? 'text-amber-400 hover:text-amber-300 font-bold'
                            : 'text-slate-400 hover:text-slate-200'
                        }`}
                        title="Modo Efecto: Abrir panel de efectos de entrada, salida y tiempo para este objeto"
                      >
                        <Zap className={`w-3 h-3 ${openSubmenu === 'animation' || isAnimActive ? 'text-amber-300 fill-amber-300/40' : 'text-slate-400'}`} />
                        <span>Efecto</span>
                        {isAnimActive && (
                    ...
                    {/* Swatches de colores para Texto o Relleno, o indicador de Efecto */}
                    <div className="flex items-center gap-1">
                      {openSubmenu === 'animation' ? (
                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-amber-950/60 border border-amber-500/40 text-[10px] font-semibold text-amber-300 shrink-0">
                          <Zap className="w-3 h-3 text-amber-400" />
                          <span>Panel de Efecto / Tiempo abierto</span>
                        </div>
        ...
        {/* ========================================================================= */}
        {/* PANEL EXPANDIBLE DE ANIMACIONES Y EFECTOS CON TIEMPO (ENTRADA Y SALIDA) */}
        {/* ========================================================================= */}
        {openSubmenu === 'animation' && (
          <div className="bg-slate-950/95 border border-amber-500/40 rounded-xl p-3 shadow-2xl space-y-3">
            {/* Header del Panel */}
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
              <div className="flex items-center gap-2">
                <Zap className="w-4 h-4 text-amber-400 fill-amber-400/20" />
                <span className="text-xs font-bold text-white">
                  Efectos de Animación & Tiempo ({activeKey})
                </span>
                <span className="text-[10px] text-amber-300 bg-amber-950/90 px-2 py-0.5 rounded-full border border-amber-600/40 font-mono">
                  Entrada: {currentInEffect !== 'none' ? `${currentInDelay}s` : 'Fijo'} | Salida: {currentOutEffect !== 'none' ? `${currentOutTime}s` : 'Hasta el final'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                {/* Botón Probar / Test Animation */}
                <button
                  type="button"
                  onClick={() => {
                    setIsPlayingAnimPreview(true);
                    onPreviewAnimation?.(activeKey, (slide.duration || 3.5));
                    setTimeout(() => setIsPlayingAnimPreview(false), ((slide.duration || 3.5) + 0.5) * 1000);
                  }}
                  disabled={isPlayingAnimPreview}
                  className="flex items-center gap-1 text-[11px] bg-amber-600 hover:bg-amber-500 text-white px-2.5 py-1 rounded-lg font-bold transition shadow-sm cursor-pointer disabled:opacity-50"
                  title="Reproducir la entrada y salida de este elemento en el lienzo ahora"
                >
                  <Play className="w-3 h-3 fill-current" />
                  <span>{isPlayingAnimPreview ? 'Reproduciendo...' : 'Probar Efecto'}</span>
                </button>
                  ...
                  <button
                    ...
                    onClick={() => {
                      onUpdateStyle(activeKey, {
                        ...
                        inDelay: 0,
                        animationOut: 'none',
                        outTime: slide.duration || 3.5,
                      });
                    }}
                    className="text-[10px] text-slate-400 hover:text-rose-300 bg-slate-900 border border-slate-800 hover:border-rose-900 px-2 py-1 rounded-lg font-semibold transition"
                    title="Quitar animaciones y dejar el objeto fijo"
                  >
                    Restablecer
                  </button>
                )}
            ...
            <div className="flex items-center gap-2 border-b border-slate-800/80 pb-2">
              ...
              >
                <span>✨ Efecto de Entrada (Aparición)</span>
                {currentInEffect !== 'none' && (
              ...
              >
                <span>🚪 Efecto de Salida (Desaparición)</span>
                {currentOutEffect !== 'none' && (
                  <span className="w-1.5 h-1.5 rounded-full bg-slate-950" />
                )}
              </button>

              <div className="ml-auto text-[11px] text-slate-400 font-mono hidden sm:block">
                Duración de Diapositiva: {(slide.duration || 3.5).toFixed(1)}s
              </div>
            </div>

            {/* CONTENIDO TAB ENTRADA */}
            {animTab === 'in' ? (
              <div className="space-y-3 pt-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5">
                    {ANIMATION_IN_PRESETS.map((preset) => {
                      const isSelected = currentInEffect === preset.id;
                      return (
                        <button
                          key={preset.id}
                          type="button"
                          onClick={() => {
                            onUpdateStyle(activeKey, {
                              animationIn: preset.id as ElementAnimationIn,
                              animationInDuration: currentInDuration,
                              inDelay: currentInDelay,
                            });
                          }}
                          className={`p-2 rounded-xl border text-left transition flex flex-col gap-0.5 ${
                            isSelected
                              ? 'bg-amber-950/90 border-amber-500 text-amber-200 ring-1 ring-amber-500/50 shadow-md'
                ...
                {/* Configuración de Tiempo de Entrada (¿Cuándo y qué tan rápido entra?) */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-800/80">
                  {/* Momento de inicio (inDelay en segundos) */}
                  <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-2.5 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <Timer className="w-3.5 h-3.5 text-amber-400" />
                        <span className="text-xs font-bold text-slate-200">
                          Momento de Entrada (Tiempo):
                        </span>
                      </div>
                      <span className="font-mono text-xs font-extrabold text-amber-300 bg-amber-950 px-2 py-0.5 rounded border border-amber-800/60">
                        {currentInDelay.toFixed(2)}s
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          const next = Math.max(0, Math.round((currentInDelay - 0.25) * 100) / 100);
                          onUpdateStyle(activeKey, { inDelay: next });
                        }}
                        className="p-1 hover:bg-slate-800 rounded text-slate-300 transition"
                        title="0.25s antes"
                      >
                      ...
                      <input
                        type="range"
                        min="0"
                        max={Math.max(1, (slide.duration || 3.5) - 0.5)}
                        step="0.05"
                        value={currentInDelay}
                        onChange={(e) => {
                          const val = Math.round(Number(e.target.value) * 100) / 100;
                          onUpdateStyle(activeKey, { inDelay: val });
                        }}
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const maxDelay = Math.max(0, (slide.duration || 3.5) - 0.5);
                          const next = Math.min(maxDelay, Math.round((currentInDelay + 0.25) * 100) / 100);
                          onUpdateStyle(activeKey, { inDelay: next });
                        }}
                        className="p-1 hover:bg-slate-800 rounded text-slate-300 transition"
                        title="0.25s después"
                    ...
                    <div className="flex items-center gap-1 flex-wrap pt-1">
                      ...
                      {[
                        ...
                        <button
                          key={preset.val}
                          type="button"
                          onClick={() => onUpdateStyle(activeKey, { inDelay: preset.val })}
                          className={`px-2 py-0.5 rounded text-[10px] font-mono font-semibold transition ${
                            Math.abs(currentInDelay - preset.val) < 0.05
                  ...
                  <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-2.5 space-y-2">
                    ...
                    <input
                      ...
                      max="1.5"
                      step="0.05"
                      value={currentInDuration}
                      onChange={(e) => {
                        const val = Math.round(Number(e.target.value) * 100) / 100;
                        onUpdateStyle(activeKey, { animationInDuration: val });
                      }}
                    ...
                    <div className="flex items-center gap-1 flex-wrap pt-1">
                      {[
                        { label: '⚡ Flash (0.25s)', val: 0.25 },
                        { label: '🚀 Rápido (0.4s)', val: 0.4 },
                        { label: '✨ Estándar (0.6s)', val: 0.6 },
                        { label: '🌊 Suave (1.0s)', val: 1.0 },
                      ].map((item) => (
                        <button
                          key={item.val}
                          type="button"
                          onClick={() => onUpdateStyle(activeKey, { animationInDuration: item.val })}
                          className={`px-2 py-0.5 rounded text-[10px] font-semibold transition ${
                            Math.abs(currentInDuration - item.val) < 0.05
                              ? 'bg-amber-600 text-white'
                              : 'bg-slate-950 text-slate-400 hover:text-white border border-slate-800'
                          }`}
                        >
              ...
              /* CONTENIDO TAB SALIDA */
              <div className="space-y-3 pt-1">
                {/* Presets Grid Salida */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-semibold text-slate-300">
                    Tipo de Efecto de Salida:
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                    {ANIMATION_OUT_PRESETS.map((preset) => {
                      const isSelected = currentOutEffect === preset.id;
                      return (
                        <button
                          key={preset.id}
                          type="button"
                          onClick={() => {
                            onUpdateStyle(activeKey, {
                              animationOut: preset.id as ElementAnimationOut,
                              animationOutDuration: currentOutDuration,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-800/80">
                  {/* Momento de salida (outTime en segundos) */}
                  <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-2.5 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <Timer className="w-3.5 h-3.5 text-amber-400" />
                        <span className="text-xs font-bold text-slate-200">
                          Momento de Salida (Tiempo):
                        </span>
                      </div>
                      <span className="font-mono text-xs font-extrabold text-amber-300 bg-amber-950 px-2 py-0.5 rounded border border-amber-800/60">
                        {currentOutEffect === 'none' ? 'Hasta el final' : `${currentOutTime.toFixed(2)}s`}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        disabled={currentOutEffect === 'none'}
                        onClick={() => {
                          const next = Math.max(0.5, Math.round((currentOutTime - 0.25) * 100) / 100);
                          onUpdateStyle(activeKey, { outTime: next });
                        }}
                        className="p-1 hover:bg-slate-800 rounded text-slate-300 transition disabled:opacity-30"
                        title="0.25s antes"
                      >
                        <ChevronLeft className="w-3.5 h-3.5" />
                      </button>
                      ...
                      <input
                        ...
                        disabled={currentOutEffect === 'none'}
                        min="0.5"
                        max={slide.duration || 3.5}
                        step="0.05"
                        value={currentOutTime}
                        onChange={(e) => {
                          const val = Math.round(Number(e.target.value) * 100) / 100;
                          onUpdateStyle(activeKey, { outTime: val });
                        }}
                        className="flex-1 accent-amber-500 cursor-pointer h-1.5 bg-slate-800 rounded-lg disabled:opacity-30"
                      />
                      ...
                      <button
                        ...
                        disabled={currentOutEffect === 'none'}
                        onClick={() => {
                          const maxTime = slide.duration || 3.5;
                          const next = Math.min(maxTime, Math.round((currentOutTime + 0.25) * 100) / 100);
                          onUpdateStyle(activeKey, { outTime: next });
                        }}
                        className="p-1 hover:bg-slate-800 rounded text-slate-300 transition disabled:opacity-30"
                        title="0.25s después"
                    ...
                    {/* Presets rápidos de salida */}
                    <div className="flex items-center gap-1 flex-wrap pt-1">
                      <span className="text-[10px] text-slate-400 mr-1">Rápido:</span>
                      {[
                        { label: 'Final', val: slide.duration || 3.5 },
                        { label: '2.0s', val: 2.0 },
                        { label: '2.5s', val: 2.5 },
                        { label: '3.0s', val: 3.0 },
                      ].map((preset) => (
                        <button
                          ...
                          type="button"
                          disabled={currentOutEffect === 'none'}
                          onClick={() => onUpdateStyle(activeKey, { outTime: preset.val })}
                          className={`px-2 py-0.5 rounded text-[10px] font-mono font-semibold transition disabled:opacity-30 ${
                            Math.abs(currentOutTime - preset.val) < 0.05
                              ? 'bg-amber-600 text-white'
                              : 'bg-slate-950 text-slate-400 hover:text-white border border-slate-800'
                          }`}
                        >
                  ...
                  <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-2.5 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-xs font-extrabold text-amber-300 bg-amber-950 px-2 py-0.5 rounded border border-amber-800/60">
                        {currentOutDuration.toFixed(2)}s
                      </span>
                    </div>
                    ...
                    <input
                      ...
                      max="1.5"
                      step="0.05"
                      value={currentOutDuration}
                      onChange={(e) => {
                        const val = Math.round(Number(e.target.value) * 100) / 100;
                        onUpdateStyle(activeKey, { animationOutDuration: val });
                      }}
                      className="w-full accent-amber-500 cursor-pointer h-1.5 bg-slate-800 rounded-lg disabled:opacity-30"
                    />
                    ...
                    <div className="flex items-center gap-1 flex-wrap pt-1">
                      {[
                        ...
                        <button
                          key={item.val}
                          type="button"
                          onClick={() => onUpdateStyle(activeKey, { animationOutDuration: item.val })}
                          className={`px-2 py-0.5 rounded text-[10px] font-semibold transition disabled:opacity-30 ${
                            Math.abs(currentOutDuration - item.val) < 0.05
                              ? 'bg-amber-600 text-white'
                              : 'bg-slate-950 text-slate-400 hover:text-white border border-slate-800'
                          }`}
                        >
