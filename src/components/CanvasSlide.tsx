...
export const CanvasSlide: React.FC<CanvasSlideProps> = ({
  ...
  // Aspect ratio map - Perfect dimensional parity with VideoStudio and VideoPreviewPlayer
  const aspectClassMap: Record<AspectRatio, string> = {
    '4:5': 'aspect-[4/5] max-h-[540px] w-full max-w-[432px]',
    '1:1': 'aspect-square max-h-[520px] w-full max-w-[520px]',
    '9:16': 'aspect-[9/16] max-h-[580px] w-full max-w-[326px]',
    '16:9': 'aspect-[16/9] max-h-[440px] w-full max-w-3xl',
  };

  // Auto-adjusting typography, spacing, and padding profiles per aspect ratio
    ...
    switch (aspectRatio) {
      case '16:9':
        return {
          ...
          titleLineHeight: 1.2,
          bodyLineHeight: 1.4,
          canvasPadding: 'px-6 sm:px-8 py-3 sm:py-3.5',
          contentSpacing: 'space-y-1.5 sm:space-y-2',
          bulletSpacing: 'space-y-1 sm:space-y-1.5',
          bulletPadding: 'px-3 py-1 sm:py-1.5',
          badgePadding: 'px-2.5 py-0.5 text-[9px]',
      ...
      case '1:1':
        return {
          ...
          statNumberSize: 52,
          statLabelSize: 12,
          statSubtextSize: 11.5,
          compTagSize: 10.5,
          compTitleSize: 12.5,
          compTextSize: 10.5,
          brandNameSize: 11.5,
          brandHandleSize: 10.5,
          brandWebSize: 10.5,
          titleLineHeight: 1.22,
          bodyLineHeight: 1.45,
          canvasPadding: 'p-4 sm:p-4.5',
          contentSpacing: 'space-y-2 sm:space-y-2.5',
          bulletSpacing: 'space-y-1.5',
          bulletPadding: 'px-3.5 py-1.5',
          badgePadding: 'px-2.5 py-0.5 text-[9.5px]',
      ...
      case '9:16':
        return {
          ...
          bodyLineHeight: 1.6,
          canvasPadding: 'px-5 py-6 sm:py-7',
          contentSpacing: 'space-y-3 sm:space-y-3.5',
          bulletSpacing: 'space-y-2',
          bulletPadding: 'px-3.5 py-2',
          badgePadding: 'px-3 py-1 text-[10px]',
  ...
  const getDefaultsForElement = (key: string, pColor: string): React.CSSProperties => {
    ...
    if (key === 'brandName') return { color: '#e2e8f0', fontSize: `${aspectScale.brandNameSize}px`, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em', zIndex: 30 };
    if (key === 'brandHandle') return { color: '#94a3b8', fontSize: `${aspectScale.brandHandleSize}px`, fontWeight: 500, letterSpacing: '0.025em', zIndex: 30 };
    if (key === 'brandWeb') return { color: pColor, fontSize: `${aspectScale.brandWebSize}px`, fontWeight: 'bold', zIndex: 30 };
    if (key === 'badge') return { color: '#ffffff', fontSize: `${aspectScale.badgeSize}px`, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em', zIndex: 28 };
    if (key === 'subtag') return { color: pColor, fontSize: `${aspectScale.subtagSize}px`, fontWeight: 'bold', zIndex: 30 };
    if (key === 'title') return { color: '#ffffff', fontSize: `${aspectScale.titleSize}px`, fontWeight: 900, textTransform: 'uppercase', lineHeight: aspectScale.titleLineHeight, zIndex: 32 };
    if (key === 'body') return { color: '#cbd5e1', fontSize: `${aspectScale.bodySize}px`, lineHeight: aspectScale.bodyLineHeight, zIndex: 30 };
    if (key.startsWith('bullet-')) return { color: '#e2e8f0', fontSize: `${aspectScale.bulletSize}px`, lineHeight: 1.45, zIndex: 30 };
    if (key === 'cta') return { color: '#cbd5e1', fontSize: `${aspectScale.ctaSize}px`, fontWeight: 600, zIndex: 30 };
    
    if (key === 'quote-icon') return { zIndex: 28 };
    if (key === 'quote-text') return { color: '#ffffff', fontSize: `${aspectScale.quoteSize}px`, fontWeight: 'bold', fontStyle: 'italic', lineHeight: 1.45, zIndex: 30 };
    if (key === 'quote-author') return { color: '#ffffff', fontSize: `${aspectScale.bodySize}px`, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em', zIndex: 30 };
    if (key === 'quote-role') return { color: '#fb7185', fontSize: `${aspectScale.subtagSize}px`, fontWeight: 600, zIndex: 30 };

    if (key === 'stat-number') return { color: pColor, fontSize: `${aspectScale.statNumberSize}px`, fontWeight: 900, lineHeight: 1, zIndex: 32 };
    if (key === 'stat-label') return { color: '#ffffff', fontSize: `${aspectScale.statLabelSize}px`, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em', zIndex: 30 };
    if (key === 'stat-subtext') return { color: '#cbd5e1', fontSize: `${aspectScale.statSubtextSize}px`, lineHeight: 1.45, zIndex: 30 };

    if (key === 'comp-leftTag') return { color: '#f87171', fontSize: `${aspectScale.compTagSize}px`, fontWeight: 900, textTransform: 'uppercase', zIndex: 28 };
    if (key === 'comp-leftTitle') return { color: '#ffffff', fontSize: `${aspectScale.compTitleSize}px`, fontWeight: 'bold', lineHeight: 1.3, zIndex: 30 };
    if (key === 'comp-leftText') return { color: '#cbd5e1', fontSize: `${aspectScale.compTextSize}px`, lineHeight: 1.35, zIndex: 30 };
    if (key === 'comp-rightTag') return { color: '#34d399', fontSize: `${aspectScale.compTagSize}px`, fontWeight: 900, textTransform: 'uppercase', zIndex: 28 };
    if (key === 'comp-rightTitle') return { color: '#ffffff', fontSize: `${aspectScale.compTitleSize}px`, fontWeight: 'bold', lineHeight: 1.3, zIndex: 30 };
    if (key === 'comp-rightText') return { color: '#cbd5e1', fontSize: `${aspectScale.compTextSize}px`, lineHeight: 1.35, zIndex: 30 };

    if (key === 'cta-avatar') return { zIndex: 35 };
    if (key === 'cta-headline') return { color: '#ffffff', fontSize: `${aspectScale.titleSize}px`, fontWeight: 900, textTransform: 'uppercase', lineHeight: aspectScale.titleLineHeight, zIndex: 32 };
    if (key === 'cta-subheadline') return { color: '#cbd5e1', fontSize: `${aspectScale.bulletSize}px`, lineHeight: 1.45, zIndex: 30 };
    if (key === 'cta-pill') return { color: '#ffffff', fontSize: `${aspectScale.bulletSize}px`, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em', zIndex: 28 };

    return { color: '#ffffff', fontSize: `${aspectScale.bodySize}px`, zIndex: 30 };
  };
  ...
  const getStyleFor = (key: string, baseStyle?: React.CSSProperties) => {
    ...
    if (fontSize) {
      const adjustedSize = aspectRatio === '4:5' || aspectRatio === '9:16'
        ? fontSize
        : Math.max(8, Math.round(fontSize * aspectScale.scaleFactor));
      styleObj.fontSize = `${adjustedSize}px`;
    }
    ...
    // Animación de Entrada / Salida con Tiempo en segundos
    const activeAnimTime =
      previewAnimationElementKey === key && previewAnimationTime !== undefined
        ? previewAnimationTime
        : currentTimeInSlide;

    if (activeAnimTime !== undefined) {
      const animItem: TextStyleItem = {
        ...custom,
        animationIn: (customTextLayer?.animationIn || custom.animationIn),
        animationInDuration: (customTextLayer?.animationInDuration || custom.animationInDuration),
        inDelay: (customTextLayer?.inDelay !== undefined ? customTextLayer.inDelay : custom.inDelay),
        animationOut: (customTextLayer?.animationOut || custom.animationOut),
        animationOutDuration: (customTextLayer?.animationOutDuration || custom.animationOutDuration),
        outTime: (customTextLayer?.outTime !== undefined ? customTextLayer.outTime : custom.outTime),
      };

      const animState = computeElementAnimation(animItem, activeAnimTime, slide.duration || 3.5);
      if (!animState.isVisible) {
        styleObj.opacity = 0;
        styleObj.pointerEvents = 'none';
      } else {
        const baseOpacity = typeof custom.opacity === 'number' ? custom.opacity : 1;
        styleObj.opacity = baseOpacity * animState.opacity;
        if (animState.cssTransform && animState.cssTransform !== 'none') {
          if (styleObj.transform && styleObj.transform !== 'none') {
            styleObj.transform = `${styleObj.transform} ${animState.cssTransform}`;
          } else {
            styleObj.transform = animState.cssTransform;
          }
        }
        if (animState.filter) {
          styleObj.filter = styleObj.filter ? `${styleObj.filter} ${animState.filter}` : animState.filter;
        }
      }
    }
  ...
  // Synchronize background video playback when scrubbing or playing
  React.useEffect(() => {
    if (currentTimeInSlide !== undefined && videoBgRef.current && videoBgRef.current.duration) {
      const targetTime = currentTimeInSlide % videoBgRef.current.duration;
      if (Math.abs(videoBgRef.current.currentTime - targetTime) > 0.25) {
        videoBgRef.current.currentTime = targetTime;
      }
    }
  }, [currentTimeInSlide]);
  ...
  return (
    ...
    >
      ...
      {slide.mediaType === 'video' && slide.image ? (
        <div data-is-media="true" className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
          <video
            ref={videoBgRef}
            src={slide.image}
            autoPlay
      ...
      <div className={`z-10 w-full h-full relative overflow-visible flex flex-col justify-between ${aspectScale.canvasPadding}`}>
        ...
        {/* ==================================================================== */}
        {layout === 'standard' && (
          <div className={`${aspectScale.contentSpacing} my-auto w-full`}>
            {!isHidden('badge') && slide.badge && (
              ...
              >
                ...
                <span
                  ...
                  className={`${aspectScale.badgePadding} rounded-md inline-block outline-none shadow-md`}
            ...
            {!isHidden('bullets-container') && slide.bullets && slide.bullets.length > 0 && (
              <div
                ...
                className={`${aspectScale.bulletSpacing} pt-1`}
                ...
                {slide.bullets.map((bullet, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative flex items-center gap-2.5 rounded-xl ${aspectScale.bulletPadding} cursor-pointer shadow-sm transition ${
              ...
              {!isHidden('badge') && slide.badge && (
                ...
                >
                  ...
                  <span
                    ...
                    suppressContentEditableWarning
                    onBlur={(e) => onUpdateField('badge', e.currentTarget.innerText)}
                    className={`${aspectScale.badgePadding} rounded uppercase tracking-wider inline-block outline-none`}
                    style={getBadgeStyle('badge')}
                  >
                    {slide.badge}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  ...
                  {!isHidden('comp-left-card') && (
                    <div
                      ...
                      className={`rounded-2xl ${aspectScale.cardPadding} space-y-1.5 text-left transition group/card ${
        ...
        {layout === 'quote' && !isHidden('quote-container') && (
          <div
            ...
            className={`${aspectScale.contentSpacing} my-auto text-center px-2 w-full`}
        ...
        {layout === 'big_number' && !isHidden('stat-container') && (
          <div
            ...
            className={`${aspectScale.contentSpacing} my-auto text-center w-full`}
            ...
            {!isHidden('badge') && slide.badge && (
              ...
              >
                ...
                <span
                  ...
                  suppressContentEditableWarning
                  onBlur={(e) => onUpdateField('badge', e.currentTarget.innerText)}
                  className={`${aspectScale.badgePadding} rounded uppercase tracking-wider inline-block shadow outline-none`}
                  style={getBadgeStyle('badge')}
                >
        ...
        {layout === 'checklist' && !isHidden('checklist-container') && (
          <div
            ...
            className={`${aspectScale.contentSpacing} my-auto w-full`}
          ...
          >
            ...
            <div className={`${aspectScale.bulletSpacing} pt-1`}>
              {(slide.bullets && slide.bullets.length > 0
                ...
                return (
                  <div
                    key={idx}
                    data-drag-key={`bullet-${idx}`}
                    className={`group relative flex items-start gap-3 rounded-2xl ${aspectScale.cardPadding} shadow-sm cursor-pointer transition ${
                      isCardBoxTransparent(`bullet-${idx}`)
                        ? 'bg-transparent border border-slate-700/40 shadow-none'
          ...
          >
              ...
              {!isHidden('badge') && slide.badge && (
                ...
                >
                  ...
                  <span
                    ...
                    suppressContentEditableWarning
                    onBlur={(e) => onUpdateField('badge', e.currentTarget.innerText)}
                    className={`${aspectScale.badgePadding} rounded-full uppercase tracking-wider inline-block shadow-sm outline-none`}
                    style={getBadgeStyle('badge')}
                  >
            ...
            {!isHidden('cta-subheadline-card') && (
              <div
                ...
                className={`rounded-2xl ${aspectScale.cardPadding} space-y-2 transition ${
